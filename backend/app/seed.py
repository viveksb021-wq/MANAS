...
def seed_database():
    ...
    try:
        ...
        person5 = Person(
            ...
            frequently_seen=False
        )
        db.add_all([person1, person2, person3, person4, person5])
        db.commit()
        db.refresh(person1)
        db.refresh(person2)
        db.refresh(person3)
        ...
        # Enroll mock embeddings
        ravi_vec = [0.15, 0.22, -0.31, 0.70, 0.45, -0.05, 0.62, 0.21, -0.11, 0.50, 0.40, -0.15, 0.38, 0.20, -0.05, 0.65]
        meera_vec = [-0.10, 0.35, 0.18, -0.25, 0.65, 0.48, -0.12, 0.58, 0.42, -0.18, 0.75, 0.10, -0.28, 0.60, 0.20, -0.10]
        arun_vec = [0.25, 0.11, -0.42, 0.88, 0.33, -0.12, 0.55, 0.19, -0.05, 0.62, 0.31, -0.22, 0.44, 0.15, -0.08, 0.77]
        sunita_vec = [-0.15, 0.41, 0.22, -0.38, 0.73, 0.52, -0.15, 0.69, 0.35, -0.12, 0.81, 0.02, -0.34, 0.55, 0.28, -0.17]

        db.add(FaceRecognitionProfile(person_id=person1.id, embedding_data=ravi_vec, confidence_threshold=0.75))
        db.add(FaceRecognitionProfile(person_id=person2.id, embedding_data=meera_vec, confidence_threshold=0.75))
        db.add(FaceRecognitionProfile(person_id=person3.id, embedding_data=arun_vec, confidence_threshold=0.75))
        db.add(FaceRecognitionProfile(person_id=person4.id, embedding_data=sunita_vec, confidence_threshold=0.75))
