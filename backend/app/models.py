...
class FaceRecognitionProfile(Base):
    ...
    embedding_data = Column(JSON, nullable=True)  # Vector embedding array
    confidence_threshold = Column(Float, default=0.75)
    sample_images_count = Column(Integer, default=1)
...
class Reminder(Base):
    ...
    status = Column(String, default="Pending")  # "Completed", "Pending", "Missed"
...
class Alert(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    patient_id = Column(Integer, ForeignKey("patient_profiles.id"))
    alert_type = Column(String, nullable=False)  # "Reminder Missed", "Low Activity", "SOS Help", "Pattern Changed"
